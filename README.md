...
CopilotHive is a **self-improving multi-agent orchestration system** powered by **[SharpCoder](https://github.com/robkaandorp/SharpCoder)** 0.19.1 (an autonomous coding agent library) and **SharpCoder.Providers** 0.19.1 (the model-provider package).

**Current version: [0.38.0](CHANGELOG.md#0380--2026-09-08)**
...
- **EF Core persistence** — Goals, pipeline state, conversations, task mappings, and related application records are persisted in a single `copilothive.db` SQLite database via Entity Framework Core (`CopilotHiveDbContext`). This durable state supports reconciliation of persisted progress, but does not make live worker attempts, completion receipts, or memory state restart-safe. `GoalStore` and `PipelineStore` use `IDbContextFactory<CopilotHiveDbContext>`. Schema reconciliation at startup creates missing tables (`EnsureSchemaUpToDate`) and applies pending EF Core migrations (`Database.MigrateAsync()`). Pre-migration backups are created via SQLite's online backup API. SQLite runs in **WAL mode** (Write-Ahead Logging) for concurrent read/write access.
...
- **Work-slot admission and completion protection** — Attempt-stamped task identities, slot ownership, registered completion admission, duplicate/abandoned rejection, and delivery rollback protect dispatch and completion; sequential phase pointers advance only through accepted completion.
- **Operator restart caveat** — Non-destructive orchestrator restart, complete durable reconciliation, and atomic worker reservation are not yet delivered. For planned restarts or upgrades, let active goals/tasks finish and prevent new dispatch during maintenance; no drain command, flag, endpoint, or automatic safety guarantee exists.
...
Failure-informed resume remains an eligible-failure branch-preserving restart through planning/Coding; branchless eligibility is iteration exhaustion, and user-cancelled goals are excluded. This release adds no active-goal budget top-ups or arbitrary exact-phase resume. Non-destructive orchestrator restart and durable in-flight recovery are not delivered: let active goals/tasks finish and prevent new dispatch before planned maintenance; no drain command is provided.
