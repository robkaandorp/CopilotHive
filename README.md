...
- **SQLite persistence** — `PipelineStore` with auto-migration for pipeline state; `GoalStore` as the primary source of truth for goals with full CRUD, search, and iteration history; release entities with full CRUD are also managed by `GoalStore`
