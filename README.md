...
- **Dashboard** — Blazor Server UI with goals browser (filterable/searchable), goal detail with iteration timeline and dependency visualization, worker status, orchestrator view (Brain + Composer stats, session reset), live logs, and configuration; configuration page displays `hive-config.yaml` with YAML syntax highlighting (keys, comments, booleans, numbers)
