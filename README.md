...
- **Composer** — conversational agent for goal decomposition and management with streaming chat UI (`/composer`); uses a persistent SharpCoder session with 8 goal-management tools (`create_goal`, `approve_goal`, `update_goal`, `delete_goal`, `cancel_goal`, `get_goal`, `list_goals`, `search_goals`) plus codebase tools (`read_file`, `glob`, `grep`), 5 git tools (`git_log`, `git_diff`, `git_show`, `git_branch`, `git_blame`), 2 web research tools (`web_search`, `web_fetch`) — available only when `OLLAMA_API_KEY` is set, phase output inspection (`get_phase_output`), and repository listing (`list_repositories`) — queries live config to show all configured repositories; full Markdown rendering (Markdig) and chat history persistence across page navigations
