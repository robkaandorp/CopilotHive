...
CopilotHive is a **self-improving multi-agent orchestration system** powered by **[SharpCoder](https://github.com/robkaandorp/SharpCoder)** 0.20.2 (an autonomous coding agent library) and **SharpCoder.Providers** 0.20.2 (the model-provider package).
...
- **Composer Session Compaction** — Manual "Compact" and "Compact 50%" buttons in the Composer chat. Full compaction summarizes the entire session (except recent messages). Partial compaction (50%) summarizes only the oldest 50% of tokens, keeping the newest 50% verbatim — gentler, preserves more recent context. Uses SharpCoder 0.20.2's `CompactOldestPercentAsync` (with SharpCoder.Providers 0.20.2). The 8,000-character guidance cap uses append-new/compress-old policy, but does not mechanically guarantee model compliance or readability.
