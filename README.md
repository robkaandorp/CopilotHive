...
- **LLM-powered Brain** — `DistributedBrain` uses SharpCoder's `CodingAgent` with per-goal Brain sessions forked from a persistent master session, read-only file access to repos, automatic context compaction, and configurable context window (`BRAIN_CONTEXT_WINDOW`). Brain dashboard metrics always reflect the master session for stable context usage readings; exact token counts from the API (`LastKnownContextTokens`) used instead of character-based heuristic estimates. **Worker context windows are configurable** per-role or globally via `hive-config.yaml` (`workers.<role>.context_window` or `orchestrator.worker_context_window`)
...
- **Knowledge Graph** — Document data model with `KnowledgeDocument`, `DocumentLink`, and related entities. `KnowledgeGraph` service with CRUD, link management, inverse queries, BFS traversal, YAML frontmatter handling. `Goal.Documents` field for associating documents with goals. Dashboard pages at `/knowledge` and `/knowledge/{DocumentId}`
