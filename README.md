...
CopilotHive is a **self-improving multi-agent orchestration system** powered by **[SharpCoder](https://github.com/robkaandorp/SharpCoder)** 0.21.0 (an autonomous coding agent library) and **SharpCoder.Providers** 0.21.0 (the model-provider package), with per-account Copilot endpoint discovery.

**Current version: [0.39.1](CHANGELOG.md#0391--2026-09-24)**
...
4. **Improve** *(optional)* — A worker (assigned the improver role) updates `agents.md` based on metrics. Exhausted guidance compression can complete as `SKIP` without publication, but only after verified workspace cleanup; preparation or publication failures can still fail the parent goal. Concurrent semantic guidance-conflict resolution is deferred to a later release.
...
- Credentials depend on the selected provider and authentication mode. With **OAuth enabled**, the admin's stored OAuth token is provisioned to workers via gRPC (`GetWorkerConfig`), so workers need no local credentials; `GH_TOKEN` is not required on the orchestrator because the Composer defers connecting until the admin signs in. Without **OAuth** (open mode), GitHub-backed workers need an operator-provided `GH_TOKEN` or `GITHUB_TOKEN`; `GetWorkerConfig` provisions a token only from the stored admin OAuth record, not from the orchestrator's environment. Local Ollama needs no token (see the no-token example), while `ollama-cloud` needs `OLLAMA_API_KEY`, provisioned from the orchestrator environment via `GetWorkerConfig`; GitHub Copilot/Models use the GitHub token alias group. `GH_TOKEN` on the orchestrator is only a fallback for orchestrator-side clients (Composer/Brain), not a worker credential.
...
2. Build and run:
   ...
   The Brain's context window is resolved from `models.available_models[].context_window` for the selected model in `hive-config.yaml`, falling back to 150,000 when not specified. Worker context windows are configured via `orchestrator.worker_context_window` (global default) or `workers.<role>.context_window` (per-role override) in `hive-config.yaml`. An unconfigured primary worker model does not fall through to the orchestrator or global model defaults.
...
The first GitHub user to sign in becomes the admin. The stored OAuth token participates in Copilot model discovery, worker task-repository assignment, and managed orchestrator target-repository Git. For configured eligible HTTPS `github.com` targets, credential selection is blank-aware: stored OAuth → `GH_TOKEN` → `GITHUB_TOKEN`. A GitHub login does not guarantee Copilot entitlement.

The LLM configuration returned by `GetWorkerConfig` is a separate provisioning path from task-repository assignment. Worker operator environment overrides win; the worker-side provisioning rules and gRPC transport caveats below still apply. Orchestrator Git uses managed `origin` handling, and credentials are not attached to arbitrary remotes or hosts. Token-in-URL, process-argument, and Git-origin limitations remain; this is not a universal secret-free transport, and revocation cleanup or live queued-task credential rotation is not promised.
...
- **Model resolution boundaries** — Configured worker model assignments are resolved from their per-role values; an unset primary worker model remains unset and does not fall through to orchestrator/global model defaults. Context-window and other metadata fallbacks remain separate concerns.
- **In-App Model Configuration** — Users can change model configuration from the Configuration dashboard page. A "Models" tab provides dropdown selectors for the orchestrator (Brain), composer, per-role workers, and compaction model, populated from the global `Models.AvailableModels` list in `hive-config.yaml`. Unset primary models and reasoning assignments remain unset rather than being replaced by orchestrator/default values; explicit reasoning `None` is distinct from unset. Existing premium/compaction empty-string clearing remains supported. Changes take effect on the next goal dispatch and are written back to `hive-config.yaml` via `ConfigModelService`. The Composer also uses this global model list. Exposed via `GET /api/config/models` and `PATCH /api/config/models`. `hive-config.yaml` and `orchestrator.agents.md` changes are picked up on the next `SyncRepoAsync` cycle without an orchestrator restart.
...
- **Worker utilization metrics** — `GET /health/utilization` endpoint provides per-role worker utilization and bottleneck detection. Capacity projections distinguish **Available** workers from workers **Awaiting-Ready** after completion; legacy `Idle` means not-busy, not assignable capacity.
- **Self-improvement loop** — the improver modifies `agents.md` based on accumulated metrics
- **EF Core persistence** — Goals, pipeline state, conversations, task mappings, completion evidence, and related application records are persisted in a single `copilothive.db` SQLite database via Entity Framework Core (`CopilotHiveDbContext`). Retained/acknowledged evidence is not processed state, recovery permission, or exactly-once effects; durable storage does not make live worker attempts restart-safe. `GoalStore` and `PipelineStore` use `IDbContextFactory<CopilotHiveDbContext>`. Schema reconciliation at startup creates missing tables (`EnsureSchemaUpToDate`) and applies pending EF Core migrations (`Database.MigrateAsync()`). Pre-migration backups are created via SQLite's online backup API. SQLite runs in **WAL mode** (Write-Ahead Logging) for concurrent read/write access.
...
- **GitHub OAuth Authentication** — When `GITHUB_OAUTH_CLIENT_ID` and `GITHUB_OAUTH_CLIENT_SECRET` environment variables are set, all dashboard pages and REST endpoints require authentication via GitHub OAuth. The first user to sign in becomes the admin (single-user model). The stored OAuth access token is used for Copilot discovery and eligible HTTPS `github.com` repository operations, with blank-aware fallback to `GH_TOKEN` then `GITHUB_TOKEN`; GitHub login alone does not guarantee Copilot entitlement. When OAuth env vars are not set, the system runs in "open mode" (no authentication, backward compatible). Login page with "Sign in with GitHub" button, user profile (avatar + username) in nav bar, logout button
...
- **Composer Session Compaction** — Manual "Compact" and "Compact 50%" buttons in the Composer chat. Full compaction summarizes the entire session (except recent messages). Partial compaction (50%) summarizes only the oldest 50% of tokens, keeping the newest 50% verbatim — gentler, preserves more recent context. Uses SharpCoder 0.21.0's `CompactOldestPercentAsync` (with SharpCoder.Providers 0.21.0). The 8,000-character guidance cap uses append-new/compress-old policy, but does not mechanically guarantee model compliance or readability.
...
- **Failure-informed goal resume** — Failed goals can be resumed with failure context and their existing feature branch preserved; this is distinct from safe recovery of unfinished worker-local work after an orchestrator restart.
- **Durable ownership and completion evidence** — Attempt-stamped task identities, instance-pinned atomic assignment claims, durable assignment context, ownership/checkpoint/rollback wiring, registered completion admission, duplicate/abandoned rejection, and delivery rollback protect dispatch and completion; sequential phase pointers advance only through accepted completion. Durable receipts retain evidence, not processed state, recovery permission, or exactly-once effects. Standalone store primitives for snapshots or inverses remain unused unless called by the live manager path.
- **Completion ACK and readiness** — Negotiated completion ACKs are scoped to the same live worker stream; upgraded workers gate ordinary post-completion Ready on a matching completion ACK when both the ACK and completion-ready-required capabilities are negotiated, while initial Ready is not gated and legacy workers behave differently. The server withholds assignment eligibility until it accepts Ready, and unacknowledged completions are retransmitted on that stream. The pre-registration unary receipt query is read-only and currently has no worker production caller. Restored active-pointer holds retain valid evidence but do not automatically resume or release; they can remain paused indefinitely, count against parallel-goal capacity, and remain checkpoint-ineligible.
- **Operator restart caveat** — v0.39.0 does not deliver full non-destructive orchestrator restart: stream/disconnect loss can cancel or drain tasks and discard worker-retained evidence; there is no recovered running/completed/idle handshake, lost-Ready recovery, or safe crash-window processing replay. For planned maintenance, stop new submissions and approvals (including automation), keep new work Draft, revert eligible Pending work to Draft, and let actual active goals and worker tasks finish. This is a manual precaution, not an atomic drain switch; there is no general drain or unhold action. Inspect and preserve relevant history, branches, and worker-local work before any destructive choice. Full integration is planned for 0.40.0, not delivered or guaranteed by a date.
- **Iteration extension** — Goals that exhaust their iteration budget can be resumed with additional iterations via the dashboard, API, or Composer tool; this is not active-hold release.
- **Cancellation, reset, and deletion** — Cancellation removes pipeline ownership, marks a user-cancelled failure, may leave a worker running, is not a pause, and cannot later use branch-preserving extend-iterations resume. Resetting Failed → Draft and approving starts a fresh run that can clear history/retry state and attempt feature-branch deletion. Delete is available only for Draft/Failed goals and is destructive. Ordinary failed-goal resume requires an eligible failed pipeline with a branch or iteration exhaustion; it is not active-hold release.
...
- **Improve outcomes** — Exhausted guidance compression can return Completed/`SKIP` with no publication after verified cleanup; preparation or publication failure can still fail the parent goal. Automatic metrics-triggered `AGENTS.md` rollback is removed. Guidance updates are capped at 8,000 characters using append-new/compress-old policy, without mechanically guaranteeing model compliance or readability.
...
- **Worker Session Persistence** — SharpCoder `AgentSession` objects are persisted per-role on the orchestrator via two gRPC RPCs (`GetSession` / `SaveSession`). Sessions are stored in SQLite as part of `GoalPipeline`, preserving session context and phase position across orchestrator restarts; this is not a guarantee of uninterrupted execution, durable completion receipts, or preservation of in-flight worker attempts/results.
...
- **Complete phase evidence and Brain inputs** — Selected phase reports retain failed-worker diagnostics, coder no-op evidence including whitespace-only reports, and archived worker narratives. Brain review, retry, replanning, planning, and clarification prompts receive the complete relevant inputs rather than clipped excerpts. Tester workers own test authoring again, and each validation command carries an explicit budget and evidence-capture guidance.
...
### Models ownership migration

`HiveConfigFile.Models` deep-copies the entire subtree on both get and set. Mutating a returned DTO or retained setter input therefore no longer updates the owner. Use the synchronized owner mutation APIs, or deliberately edit a detached copy and assign the whole `Models` property back for replacement; copy/edit/reassign is **not** an atomic concurrent read-modify-write. Standalone `ModelsConfig` and `ModelEntry` DTOs remain mutable, YAML schema and value semantics are unchanged (this is not a mandatory YAML migration), and cache loads return detached configurations rather than shared identities.

### Catalog and cache operations

Catalog reads and writes are snapshot-backed; YAML/cache generation uses one detached configuration, and each manager serializes load, write, and sync operations. Cache invalidation occurs conservatively before reset/commit pull-recovery, including partial failures. The seven owner catalog mutation methods skip null placeholders without removing or normalizing them; this null-placeholder behavior is limited to those methods and does not claim that every malformed-data facade/parser path is null-safe.

Configuration ownership isolation, detached catalog snapshots, serialized individual operations, and conservative cache invalidation do not make write+commit or later maintenance snapshot application a transaction. Failed-sync freshness, subprocess cancellation lifetime, failed-write disk integrity, other processes/managers, and unrelated mutable configuration sections remain outside these fixes; there are no universal cache-freshness, thread-safety, crash-atomicity, or exactly-once guarantees. Complete NEW tester reports are preserved for storage and reviewer `get_test_report` handoff after the updated orchestrator is running, but historical clipped text is not recovered and other evidence-truncation/prompt-preview limits remain.

Failure-informed resume remains an eligible-failure branch-preserving restart through planning/Coding; branchless eligibility is iteration exhaustion, and user-cancelled goals are excluded. This release adds no active-goal budget top-ups or arbitrary exact-phase resume. Stored or acknowledged evidence is not processed state, recovery permission, or exactly-once effects, and the unary receipt query has no worker production caller. Held nonterminal active pointers retain valid evidence but do not resume or release automatically, may remain paused indefinitely, count against parallel-goal capacity, and remain checkpoint-ineligible. There is no recovered running/completed/idle handshake, lost-Ready recovery, or safe crash-window processing replay. Full non-destructive restart integration is planned for 0.40.0, not delivered or guaranteed by a date.

For planned maintenance, stop new submissions and approvals, including automation; keep new work Draft, revert eligible Pending work to Draft, and let actual active goals and worker tasks finish. This is a manual precaution, not an atomic drain switch, and there is no general drain or unhold action. Inspect and preserve relevant history, branches, and worker-local work before choosing cancellation, reset, or deletion. Cancellation removes pipeline ownership, marks a user-cancelled failure, may leave a worker running, is not a pause, and cannot later use branch-preserving extend-iterations resume. Failed → Draft reset followed by approval is a fresh run that can clear history and pipeline retry state and attempt feature-branch deletion. Delete is Draft/Failed only and destructive. Ordinary failed-goal resume applies only to an eligible failed pipeline with a branch or to iteration exhaustion; it is not active-hold release.
