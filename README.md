...
- **Full In-App Configuration** — All `hive-config.yaml` settings are editable from the dashboard Configuration page. New tabs: Available Models (add/edit/remove models, browse provider models from GitHub Copilot and Ollama APIs, per-model reasoning effort), Repositories (add/edit/remove with auto-clone), Orchestrator settings (max_iterations, max_retries, max_parallel_goals, verbose_logging, brain_max_steps, branch_cleanup_delay_hours), Worker context windows (per-role), and Composer settings (max_steps). Changes are written back to `hive-config.yaml` and hot-reloaded
...
- **Composer Background-tasks panel** — Live sub-agent progress (Running → terminal transitions) is displayed in the Composer chat, driven by SharpCoder's `SubAgentChanged` event.
