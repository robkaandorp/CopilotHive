...
- **Operational completion protocol** — Completion receipts are durable retained evidence, not processed state, recovery permission, or exactly-once effects. Completion ACKs are negotiated on and scoped to the same live stream; upgraded workers gate ordinary post-completion Ready on a matching completion ACK when both the ACK and completion-ready-required capabilities are negotiated, while initial Ready is not gated and legacy workers behave differently. The server withholds assignment eligibility until it accepts Ready, and unacknowledged completion is retransmitted on that stream. Stream/disconnect loss can cancel or drain tasks and discard worker-retained evidence.
