...
### Added
- **Dependency-aware goal dispatch** — `GoalManager.GetNextGoalAsync` now filters out pending goals whose `DependsOn` dependencies are not yet `Completed`; blocked goals are skipped and logged at Debug level with their unsatisfied dependency IDs; includes 8 xUnit tests covering blocked goal skipping, unblocked goal dispatching, multi-dependency scenarios, non-existent dependency blocking, empty dependencies, priority ordering among eligible goals, and debug logging verification
