...
/// <summary>
/// Tests for <see cref="GoalDispatcher.MapToPhaseInput"/> which maps Brain actions
/// to state machine inputs.
/// </summary>
public class MapToPhaseInputTests
{
    [Theory]
    [InlineData(OrchestratorActionType.Done, GoalPhase.Coding, PhaseInput.Succeeded)]
    [InlineData(OrchestratorActionType.Done, GoalPhase.Testing, PhaseInput.Succeeded)]
    [InlineData(OrchestratorActionType.Done, GoalPhase.Review, PhaseInput.Succeeded)]
    [InlineData(OrchestratorActionType.Skip, GoalPhase.Coding, PhaseInput.Succeeded)]
    [InlineData(OrchestratorActionType.Merge, GoalPhase.Review, PhaseInput.Succeeded)]
    public void Done_Skip_Merge_MapToSucceeded(OrchestratorActionType action, GoalPhase phase, PhaseInput expected)
    {
        var result = GoalDispatcher.MapToPhaseInput(action, phase);
        Assert.Equal(expected, result);
    }

    [Theory]
    [InlineData(OrchestratorActionType.RequestChanges, GoalPhase.Review)]
    [InlineData(OrchestratorActionType.RequestChanges, GoalPhase.DocWriting)]
    public void RequestChanges_MapsToRequestChanges(OrchestratorActionType action, GoalPhase phase)
    {
        Assert.Equal(PhaseInput.RequestChanges, GoalDispatcher.MapToPhaseInput(action, phase));
    }

    [Theory]
    [InlineData(OrchestratorActionType.Retry, GoalPhase.Testing)]
    [InlineData(OrchestratorActionType.Retry, GoalPhase.Coding)]
    public void Retry_MapsToFailed(OrchestratorActionType action, GoalPhase phase)
    {
        Assert.Equal(PhaseInput.Failed, GoalDispatcher.MapToPhaseInput(action, phase));
    }

    [Fact]
    public void SpawnCoder_AtCoding_ReturnsNull_NoTransition()
    {
        var result = GoalDispatcher.MapToPhaseInput(OrchestratorActionType.SpawnCoder, GoalPhase.Coding);
        Assert.Null(result);
    }
    ...
    [InlineData(GoalPhase.Review)]
    [InlineData(GoalPhase.DocWriting)]
    public void SpawnCoder_AtOtherPhase_MapsToFailed(GoalPhase phase)
    {
        Assert.Equal(PhaseInput.Failed, GoalDispatcher.MapToPhaseInput(OrchestratorActionType.SpawnCoder, phase));
    }

    [Theory]
    [InlineData(OrchestratorActionType.SpawnTester)]
    [InlineData(OrchestratorActionType.SpawnReviewer)]
    [InlineData(OrchestratorActionType.SpawnDocWriter)]
    [InlineData(OrchestratorActionType.SpawnImprover)]
    public void SpawnOtherWorker_MapsToSucceeded(OrchestratorActionType action)
    {
        Assert.Equal(PhaseInput.Succeeded, GoalDispatcher.MapToPhaseInput(action, GoalPhase.Coding));
    }
}
