...
/// <summary>
/// Cancellation contract of the Brain repo manager's SINGLE real-process git runner
/// (<c>RunGitCoreAsync</c>, the one runner every git invocation of <see cref="BrainRepoManager"/>
/// goes through): a cancelled caller must not be able to leave a git process mutating the clone
/// after the call returns.
/// <para>
/// Driven against a REAL git process — the <c>gitRunner</c> seam is deliberately left <c>null</c>
/// so the production process path (and its cancellation cleanup) is what runs. Because the seam
/// would never execute a blocking alias at all, the very appearance of the child's PID proves the
/// real-process runner was taken.
/// </para>
/// <para>
...
/// </para>
/// </summary>
public sealed class BrainRepoManagerCancellationTests : IDisposable
{
    private readonly string _tempDir;

    public BrainRepoManagerCancellationTests()
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"copilothive-braincancel-{Guid.NewGuid():N}");
    ...
    /// <summary>
    /// Cancelling a Brain git call propagates the caller's
    ...
    public async Task RunGitCoreAsync_CancelledWhileGitRuns_TerminatesTheGitProcessTree()
    {
        ...
        // Premise: no runner seam — the blocking alias below is only ever reached by the real
        // process path, so the PID file's appearance is itself the proof that the real-process
        // runner ran (the seam never launches git at all).
        var manager = new BrainRepoManager(
            _tempDir, NullLogger<BrainRepoManager>.Instance, gitRunner: null);
