...
/// <summary>
/// Verifies that the SharpCoder package reference is pinned to 0.19.0, which is required for the
...
{
    [Fact]
    public void SharpCoder_PackageReference_IsVersion0_19_0()
    {
        ...
        Assert.Equal("0.19.0", version);
    }

    /// <summary>
    /// <c>SharpCoder.Providers</c> must be pinned to 0.19.0. <c>ChatClientFactory.IsTokenAvailable</c>
    ...
    /// </summary>
    [Fact]
    public void SharpCoderProviders_PackageReference_IsVersion0_19_0()
    {
        var repoRoot = Path.GetFullPath(
            Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", ".."));
        var propsFile = Path.Combine(repoRoot, "Directory.Packages.props");
        ...
        Assert.Equal("0.19.0", providers!.Attribute("Version")?.Value);
    }
