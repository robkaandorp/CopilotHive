...
{
    private readonly ConcurrentDictionary<string, LlmSessionInfo> _sessions = new();
    ...
    public void RegisterOrUpdate(LlmSessionInfo session) => _sessions[session.SessionId] = session;
    ...
    public bool Unregister(string sessionId) => _sessions.TryRemove(sessionId, out _);

    /// <summary>Returns a snapshot of all registered sessions.</summary>
    /// <returns>A list containing every registered session.</returns>
    public List<LlmSessionInfo> GetAll() => _sessions.Values.ToList();
    ...
    public void CleanupStale(TimeSpan maxAge)
    {
        ...
        foreach (var kvp in _sessions)
        {
            if (kvp.Value.LastActivity < cutoff)
            {
                ...
                ((ICollection<KeyValuePair<string, LlmSessionInfo>>)_sessions).Remove(kvp);
            }
        }
    }
}
